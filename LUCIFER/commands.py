...
async def start(client, message):
    if message.chat.type in [enums.ChatType.GROUP, enums.ChatType.SUPERGROUP]:
        buttons = [[
                    ...
                    InlineKeyboardButton('Sᴜᴘᴘᴏʀᴛ', url='https://t.me/MV_Mania')
                  ]]
    ...
    if AUTH_CHANNEL and not await is_subscribed(client, message):
        ...
        await client.send_photo(
            photo="https://te.legra.ph/file/2db1f02eab2699969252a.jpg",
...
async def log_file(bot, message):
    """Send log file"""
    try:
        await message.reply_document('WednesdayBotLog.txt')
    except Exception as e:
...
async def settings(client, message):
    ...
    if not userid:
        return await message.reply(f"**Yᴏᴜ Aʀᴇ Aɴᴏɴʏᴍᴏᴜs Aᴅᴍɪɴ. Usᴇ /settings Iɴ PM**")
    ...
    if settings is not None:
        ...
        stng = await message.reply_text(
            text=f"<u><b>Cᴜʀʀᴇɴᴛ Sᴇᴛᴛɪɴɢs Fᴏʀ {title}</b></u>\n\n<b>Yᴏᴜ Cᴀɴ Cʜᴀɴɢᴇ Sᴇᴛᴛɪɴɢs As Yᴏᴜʀ Wɪsʜ Bʏ Usɪɴɢ Bᴇʟᴏᴡ Bᴜᴛᴛᴏɴs</b>",
            reply_markup=reply_markup,
...
async def ping(_, message):
    kd = await message.reply_sticker('CAACAgUAAxkBAAEG-t1jprpjONT96Ohp_18nF5CqLgZV-gAClQADBalSM12yeTndlfJ7LAQ')
